...
在模型输出中，一般我们还可以看到修正以后的$$R_{adj}^{2}=1-\frac{MSE}{MST}=1-\frac{SSE/(n-2)}{MST/(n-1)}$$，会比$$R^{2}$$小一点点，毕竟n-2 < n-1 （对一元模型来说这个值不重要，在多元模型中就不仅仅是n-2了，那是n-k-1，对多元模型来说这个值同时考虑了精准度和模型复杂程度，在我们取舍$$x_{i}$$的时候有用处。）    
