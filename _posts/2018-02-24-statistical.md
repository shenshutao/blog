...
## 双变量情况
###在总体方差已知的情况下，推测两个分布的均值的差别
两个分布的总体均值的差别为：$$\mu_{1}-\mu_{2}$$,因为中心极限定理两个样本均值分别服从正态分布，有意思的是，两个正态分布的差值是另外一个正态分布，均值为$$\mu_{1}-\mu_{2}$$，方差为$$\frac{\sigma_{1}^{2}}{n_{1}}+\frac{\sigma_{2}^{2}}{n_{2}}$$。         
