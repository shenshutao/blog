...
- I类错误     
    ...
    - 样本太小了？
    - 定义的拒绝域太大了？
    在这个情况下，我们引入**显著性水平(Significant Level)**的概念，$$ \color{red}{\alpha = \mathbb{P}\left [ 拒绝H_{0} | H_{0}正确 \right ] }$$ ，即发生I类错误的概率，来帮助我们定义拒绝域。  
	...
	（当样本数量n变大，样本均值们所遵从的正态分布$$\frac{\sigma^2}{n}$$的方差变小，均值们会更向中间集中，求得的显著性水平$$\alpha$$会变小，即发生I类错误的可能性变小）
...
- II类错误    
    ...
    $$ 在同样的前提假设：总体方差\sigma=2.5,样本数量n=10，我们可以根据中心极限定理获得样本均值的正态分布的方差没有变依然为0.79。$$    
    $$ 转化为标准正态分布变量时，由于\mu变为52，变成大于\frac{48.5-52}{0.79} 和 小于\frac{51.5-52}{0.79} $$
    $$ \beta = \mathbb{P}( -4.43 < Z < -0.633 ) =  \Phi(-0.63) - \Phi(-4.43) = 0.2676 - 0 = 0.2676 $$     
    当$$\mu=50.5,n=16,\sigma=2.5的情况下,我们可以求得均值的正态分布方差为0.39,\beta = \mathbb{P}( \frac{48.5-50.5}{0.39} < Z < \frac{51.5-50.5}{0.39} ) = \mathbb{P}( -5.13 < Z < 2.56 ) =  \Phi(2.56) - \Phi(-5.13) =0.9948 - 0 = 0.9948 $$ 在真实均值距离H0比较近的时候，我们没有拒绝H0的可能性极大。

- 我们也可以给出**显著性水平(Significant Level)** 的值，比如$$\alpha=0.05$$，然后根据该值和均值的正态分布函数，来求到相应的样本均值拒绝域。
